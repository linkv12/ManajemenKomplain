...
import manajemenkomplain.pengguna.client.controllerClient;
import manajemenkomplain.pengguna.manajer.controllerManajer;
...
public class controllerLogin implements ActionListener{
    ...
    private void btnLoginActionPerformed() {
        ...
        if (login != null) {
            if (login.getIdLevel().equals("usr")) {
                emptyTf();
                new controllerClient (new UserDatabase().getUser(login.getIdUser()),view);
            } else if (login.getIdLevel().equals("adm")){
                emptyTf();
                new controllerAdmin (new UserDatabase().getUser(login.getIdUser()),view);
            } else {
                emptyTf();
                new controllerManajer (new UserDatabase().getUser(login.getIdUser()),view);
            }
            emptyTf();
